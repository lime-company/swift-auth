...
public protocol ScanRecoveryCodeRoutingLogic {
    
    func routeToPreviousScene()
    func routeToEnterCode()
    func routeToKeyExchange(data: LimeAuthRecoveryData)
    
    func prepare(for segue: UIStoryboardSegue, sender: Any?)
}

public class ScanRecoveryCodeRouter: ScanRecoveryCodeRoutingLogic, ActivationUIProcessRouter {
    
    public weak var viewController: ScanRecoveryCodeViewController?
    ...
    public func routeToKeyExchange(data: LimeAuthRecoveryData) {
        activationProcess.activationData.activationCode = data.activationCode
        activationProcess.activationData.puk = data.puk
        activationProcess.activationData.originalActivationId = data.originalActivationId
